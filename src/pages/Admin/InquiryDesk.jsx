            ...
            <AnimatePresence>
                {selectedUser && (
                    <>
                        ...
                        >
                            <button onClick={() => setSelectedUser(null)} className="absolute top-4 right-4 p-2 bg-black/5 hover:bg-black/10 rounded-full text-foreground transition-colors z-20">
                                <X className="h-5 w-5" />
                            </button>
                            ...
                            <div className="w-full md:w-2/5 bg-white p-6 md:p-8 border-b md:border-b-0 md:border-r border-border/40 flex flex-col items-center text-center">
                                <div className={`h-24 w-24 rounded-full flex items-center justify-center text-3xl font-black text-white shadow-lg mb-4 ${selectedUser.crmStatus === 'banned' ? 'bg-red-500' : 'bg-[#2A5244]'}`}>
                                    {selectedUser.photo}
                                </div>
                                <h2 className="text-xl font-bold font-serif text-foreground">{selectedUser.name}</h2>
                                <span className={`mt-2 text-xs font-black uppercase tracking-wider px-3 py-1 rounded-full border ${selectedUser.leadScore === 'Hot' ? 'bg-[#FA6D16]/10 text-[#FA6D16] border-[#FA6D16]/20' :
                                    selectedUser.leadScore === 'Spam' ? 'bg-red-100 text-red-600 border-red-200' : 'bg-blue-100 text-blue-600 border-blue-200'
                                    }`}>
                                    {selectedUser.leadScore} Lead
                                </span>

                                <div className="mt-6 w-full space-y-3 text-sm text-left">
                                    <p className="flex items-center gap-2 text-muted-foreground font-medium"><Mail className="h-4 w-4 shrink-0 text-[#2A5244]" /> <span className="truncate">{selectedUser.email}</span></p>
                                    <p className="flex items-center gap-2 text-muted-foreground font-medium"><Smartphone className="h-4 w-4 shrink-0 text-[#2A5244]" /> {selectedUser.phone}</p>
                                    {selectedUser.location && (
                                        <p className="flex items-center gap-2 text-muted-foreground font-medium"><MapPin className="h-4 w-4 shrink-0 text-[#2A5244]" /> {selectedUser.location}</p>
                                    )}
                                </div>

                                <div className="mt-8 pt-6 w-full border-t border-border/40 md:mt-auto md:pt-8">
                                    {selectedUser.crmStatus === 'active' ? (
                            ...
                            <div className="w-full md:w-3/5 p-6 md:p-8 bg-[#FDFBF7]">
                                ...
                                <div className="grid grid-cols-2 gap-3 md:gap-4">
                                    <div className="bg-white p-4 rounded-xl border border-border/40 shadow-sm">
                                        ...
                                        <p className="text-2xl font-black text-[#2A5244]">{selectedUser.totalVisits}</p>
                                    </div>
                                    <div className="bg-white p-4 rounded-xl border border-border/40 shadow-sm">
                                        <p className="text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Last Seen</p>
                                        <p className="text-sm md:text-base font-bold text-foreground mt-1.5">{selectedUser.lastSeen}</p>
                                    </div>
                                    <div className="col-span-2 bg-white p-4 rounded-xl border border-border/40 shadow-sm">
                                        ...
                                        <p className="text-sm md:text-base font-bold text-foreground flex items-center gap-2 mt-1">
                                            <Compass className="h-4 w-4 text-[#FA6D16] shrink-0" /> <span className="truncate">{selectedUser.mostViewed}</span>
                                        </p>
                                    </div>
                                    <div className="col-span-2 bg-white p-4 rounded-xl border border-border/40 shadow-sm">
                                        <p className="text-[10px] md:text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Highest Interest Vibe</p>
                                        <p className="text-sm md:text-base font-bold text-foreground flex items-center gap-2 mt-1">
                                            <TrendingUp className="h-4 w-4 text-[#2A5244] shrink-0" /> {selectedUser.topVibe}
                                        </p>
                                    </div>
                                </div>
                            </div>
