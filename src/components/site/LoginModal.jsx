...
export function LoginModal({ trigger, open: controlledOpen, onOpenChange: setControlledOpen }) {
    ...
    const validate = () => {
        ...
        if (!formData.password) {
            newErrors.password = "Your password must contain between 4 and 60 chars.";
            ...
            <DialogContent className="fixed left-[50%] top-[50%] z-50 w-[calc(100%-2rem)] max-w-120 -translate-x-1/2 -translate-y-1/2 p-0 overflow-hidden rounded-3xl border border-border/50 bg-background shadow-2xl duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-[0.98] data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
                ...
                <div className="max-h-[calc(100dvh-2rem)] overflow-y-auto px-8 py-6 sm:px-10 sm:py-8 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-border/60 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-border">
                        ...
                        <div className={`grid transition-[grid-template-rows,opacity] duration-500 ease-in-out ${isLogin ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
                            <div className="overflow-hidden">
                                <div className="flex w-full pb-2 pl-1">
                                    <button type="button" className="text-[13px] font-bold text-foreground hover:underline hover:text-[#FA6D16] transition-colors cursor-pointer">
                    ...
                    <div className="mt-5 text-center text-sm">
                        {isLogin ? (
                            <p className="text-muted-foreground">New here? <button onClick={() => { setIsLogin(false); resetForm(); }} className="font-bold text-foreground hover:underline hover:text-[#FA6D16] transition-colors cursor-pointer">Sign Up</button></p>
                        ) : (
                            <p className="text-muted-foreground">Already a member? <button onClick={() => { setIsLogin(true); resetForm(); }} className="font-bold text-foreground hover:underline hover:text-[#FA6D16] transition-colors cursor-pointer">Log in</button></p>
                        )}
                    </div>
                </div>
            </DialogContent>
